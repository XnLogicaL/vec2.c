...
// geometric
#define vec2_magnitude(v) (sqrt(vec2_x(v) * vec2_x(v) + vec2_y(v) * vec2_y(v)))
#define vec2_unit(v) (v / vec2_magnitude(v))
#define vec2_angle(v) (atan2(vec2_y(v), vec2_x(v)))

// stuff
#define vec2_dot(v1, v2) vec2_x((v1)*conj(v2))
#define vec2_cross(v1, v2) (vec2_y((v1)*conj(v2)))
#define vec2_proj(v, onto) ((vec2_dot(v, onto) / vec2_dot(onto, onto)) * (onto))
